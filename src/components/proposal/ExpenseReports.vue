    ...
    <q-markup-table flat bordered separator="cell">
      ...
      <tbody>
        <tr v-if="isEditing">
          ...
          <td class="text-left">
            <select
              id="budget_category_selector"
              v-model="formData.budget_category"
            >
              <option
                :value="cat.id"
          ...
          <td class="text-right">
            {{ $commaSeparator(formData.unit_cost * formData.quantity) }} UGX
          </td>
          <td class="text-right">
          ...
          <td class="text-left">
            ...
            />
            <q-spinner v-else color="primary" size="1.5rem" :thickness="5" />
          </td>
        </tr>
...
export default {
  ...
  methods: {
    getExpenditures() {
      this.$api
        .get(`expenditures/?proposal=${this.$route.params.proposal_id}`)
        .then((res) => {
